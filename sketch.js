...
function swap(i, j, arr, isMouseClick) {
  let temp = arr[i];
  arr[i] = arr[j];
  arr[j] = temp;
  if(isMouseClick) {
    noofMoves++
    document.getElementById('moves').innerHTML = noofMoves
  }
}
...
// Swap two pieces
function move(i, j, arr, isMouseClick=false) {
  let blank = findBlank();
  ...
  if (isNeighbor(i, j, blankCol, blankRow)) {
    swap(blank, i + j * cols, arr, isMouseClick);
  }
}
...
//   // Add any initialization or logic needed to start the game
//   gameStarted = true;
//   setup();
// }
